...
public class Tester  {
	  ...
	  System.out.println(mtr.substring(2));
		 ...
		 * setHeader("Content-Type","application/json"). setBody(jsonContent)
		 * .execute(new AsyncHandler<Integer>() { private Integer status;
		 * 
		 * @Override public State onStatusReceived(HttpResponseStatus responseStatus)
		 * throws Exception { status = responseStatus.getStatusCode(); return
		 * State.ABORT; }
		 * 
		 * @Override public State onHeadersReceived(HttpHeaders headers) throws
		 * Exception { return State.ABORT; }
		 * 
		 * @Override public State onBodyPartReceived(HttpResponseBodyPart bodyPart)
		 * throws Exception { return State.ABORT; }
		 * 
		 * @Override public Integer onCompleted() throws Exception {
		 * 
